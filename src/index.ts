						...
						try {
							...
							return await prisma.$transaction(
								async (tx) => {
									...
									for (const [key, value] of toPairs(sanitizedContext)) {
										await tx.$executeRaw`SELECT set_config(${key}, ${value.toString()}, true);`;
									}

									// Execute the captured request inside this transaction without re-running
									// any query extensions that ran before Yates.
									return await (tx as any)._createPrismaPromise(
										(transaction: unknown) =>
											(prisma as any)._executeRequest({
												...requestParams,
												transaction,
											}),
									);
