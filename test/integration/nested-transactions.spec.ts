...
describe("nested transactions", () => {
	it("should apply restrictive context without leaking it between nested queries", async () => {
		const admin = createPrismaClient();
		const role = `USER_${uuid()}`;
		const allowedTitle = `allowed-${uuid()}`;
		const deniedTitle = `denied-${uuid()}`;
		await admin.post.createMany({
			data: [{ title: allowedTitle }, { title: deniedTitle }],
		});
		...
		let currentContext: {
			...
			role,
			context: { "post.title": allowedTitle },
		};
		const client = await setup({
			...
			customAbilities: {
				Post: {
					readMatchingTitle: {
						description: "Read posts matching the current context title",
						operation: "SELECT",
						expression: "current_setting('post.title', true) = title",
					},
				},
			},
			getRoles(abilities) {
				return {
					[role]: [abilities.Post.readMatchingTitle],
				};
			},
		...
		await client.$transaction(async (outerTx) => {
			await outerTx.$transaction(async (innerTx) => {
				const allowed = await innerTx.post.findMany({
					where: { title: { in: [allowedTitle, deniedTitle] } },
				});
				expect(allowed.map(({ title }) => title)).toEqual([allowedTitle]);

				currentContext = { role, context: {} };
				const withoutTitleContext = await innerTx.post.findMany({
					where: { title: { in: [allowedTitle, deniedTitle] } },
				});
				expect(withoutTitleContext).toEqual([]);

				currentContext = null;
				const bypassed = await innerTx.post.findMany({
					where: { title: { in: [allowedTitle, deniedTitle] } },
					orderBy: { title: "asc" },
				});
				expect(bypassed.map(({ title }) => title)).toEqual(
					[allowedTitle, deniedTitle].sort(),
				);
			});
		});
	});

	it("should forward transaction limits to Yates query transactions", async () => {
		...
		const client = await setup({
			...
			options: { txMaxWait: 1234, txTimeout: 5678 },
		});
		transactionSpy.mockClear();

		await client.account.findMany();

		expect(transactionSpy).toHaveBeenCalledWith(expect.anything(), {
			maxWait: 1234,
			timeout: 5678,
		});
	});
