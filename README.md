...
`txMaxWait` and `txTimeout` apply to transactions that Yates opens for standalone queries. For an explicit interactive transaction, pass `maxWait` and `timeout` to Prisma's `$transaction` call; Yates reuses the caller-owned transaction.
